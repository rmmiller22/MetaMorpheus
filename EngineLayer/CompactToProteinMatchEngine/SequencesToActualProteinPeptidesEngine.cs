...
namespace EngineLayer
{
    public class SequencesToActualProteinPeptidesEngine : MetaMorpheusEngine
    {
        ...
        protected static void ResolveAmbiguities(Dictionary<CompactPeptideBase, HashSet<PepsWSMods>> compactPeptideToProteinPeptideMatching)
        {
            ...
            {
                HashSet<PepsWSMods> value = compactPeptideToProteinPeptideMatching[key];
                compactPeptideToProteinPeptideMatching[key] = new HashSet<PepsWSMods> { value.FirstOrDefault(b => !b.Protein.IsDecoy) ?? value.First() };
            }
        }
                ...
                if (psm != null)
                {
                    foreach (var compactPeptide in psm.CompactPeptides)
                    {
                        if (!compactPeptideToProteinPeptideMatching.ContainsKey(compactPeptide.Key))
                            compactPeptideToProteinPeptideMatching.Add(compactPeptide.Key, new HashSet<PepsWSMods>());
                    }
                }
            }
