...
namespace EngineLayer.NonSpecificEnzymeSearch
{
    public class NonSpecificEnzymeSequencesToActualPeptides : SequencesToActualProteinPeptidesEngine
    {
        ...
        private readonly Dictionary<CompactPeptideBase, HashSet<PepsWSMods>> CPWMtoPWSM;

        public NonSpecificEnzymeSequencesToActualPeptides(Dictionary<CompactPeptideBase, HashSet<PepsWSMods>> CPWMtoPWSM, List<PeptideSpectralMatch> allPsms, List<Protein> proteinList, List<ModificationWithMass> fixedModifications, List<ModificationWithMass> variableModifications, List<ProductType> ionTypes, IEnumerable<DigestionParams> CollectionOfDigestionParams, MassDiffAcceptor massDiffAcceptor, bool reportAllAmbiguity, CommonParameters commonParameters, List<string> nestedIds) : base(allPsms, proteinList, fixedModifications, variableModifications, ionTypes, CollectionOfDigestionParams, reportAllAmbiguity, commonParameters, nestedIds)
        {
        ...
        protected override MetaMorpheusEngineResults RunSpecific()
        {
            ...
            foreach (var psm in AllPsms.Where(psm => psm != null))
            {
                foreach (var cp in psm.CompactPeptides)
                {
                    ...
                    {
                        compactPeptideToProteinPeptideMatching.Add(cp.Key as CompactPeptideBase, new HashSet<PepsWSMods>()); //populate dictionary with all keys
            ...
            if (TerminusType == TerminusType.N)
            {
                Parallel.ForEach(Partitioner.Create(0, totalProteins), new ParallelOptions { MaxDegreeOfParallelism = commonParameters.MaxThreadsToUsePerFile }, fff =>
                {
                    ...
                    Dictionary<CompactPeptideBase, HashSet<PepsWSMods>> localCPtoPWSM = compactPeptideToProteinPeptideMatching.ToDictionary(b => b.Key as CompactPeptideBase, b => new HashSet<PepsWSMods>());
                    for (int i = fff.Item1; i < fff.Item2; i++)
                    {
                        foreach (var digestionParam in CollectionOfDigestionParams)
                        {
                            ...
                            {
                                PepsWSMods pepWSMods = new PepsWSMods(peptideWithSetModifications.Protein, peptideWithSetModifications.DigestionParams, peptideWithSetModifications.OneBasedStartResidueInProtein, peptideWithSetModifications.OneBasedEndResidueInProtein, peptideWithSetModifications.PeptideDescription, peptideWithSetModifications.MissedCleavages, peptideWithSetModifications.AllModsOneIsNterminus, peptideWithSetModifications.NumFixedMods);
                                if (localCPtoPWSM.TryGetValue(new CompactPeptide(peptideWithSetModifications, TerminusType), out HashSet<PepsWSMods> v))
                                {
                                    v.Add(pepWSMods);
                                }
                            }
                    ...
                    Dictionary<CompactPeptideWithModifiedMass, HashSet<PepsWSMods>> localCPWMtoPWSM = new Dictionary<CompactPeptideWithModifiedMass, HashSet<PepsWSMods>>();
                    foreach (KeyValuePair<CompactPeptideBase, HashSet<PepsWSMods>> kvp in localCPtoPWSM) //foreach theoretical kvp
                    {
                        ...
                        {
                            foreach (PepsWSMods pwsm in kvp.Value)
                            {
                                    ...
                                    foreach (double precursorMass in listScanPrecursorMasses) //foreach precursor
                                    {
                                        ...
                                        foreach (DigestionParams digestionParam in CollectionOfDigestionParams)
                                        {
                                            if (index >= 0 && index >= digestionParam.MinPeptideLength)
                                            {
                                                ...
                                                PepsWSMods tempPWSM = new PepsWSMods(pwsm.Protein, pwsm.DigestionParams, pwsm.OneBasedStartResidueInProtein, pwsm.OneBasedStartResidueInProtein + index - 1, pwsm.PeptideDescription, pwsm.MissedCleavages, allModsOneIsNTerminus, pwsm.NumFixedMods);
                                                ...
                                                if (localCPWMtoPWSM.TryGetValue(tempCPWM, out HashSet<PepsWSMods> tempPWSMHashSet))
                                                {
                                                    tempPWSMHashSet.Add(tempPWSM);
                                                }
                                                else
                                                {
                                                    localCPWMtoPWSM.Add(tempCPWM, new HashSet<PepsWSMods> { tempPWSM });
                                                }
                                            }
                                        }
                                    }
                    ...
                    {
                        //PopulateCPWMtoPWSM
                        foreach (KeyValuePair<CompactPeptideWithModifiedMass, HashSet<PepsWSMods>> kvp in localCPWMtoPWSM)
                        {
                            if (CPWMtoPWSM.TryGetValue(kvp.Key, out HashSet<PepsWSMods> tempPWSMHashSet))
                            {
                                foreach (PepsWSMods PWSM in kvp.Value)
                                {
                            ...
                            {
                                tempPWSMHashSet = new HashSet<PepsWSMods>();
                                foreach (PepsWSMods PWSM in kvp.Value)
                                {
                                    if (!tempPWSMHashSet.Contains(PWSM))
                                    {
            ...
            else //if (terminusType==TerminusType.C)
            {
                ...
                {
                    //Digest protein into large peptide fragments and store in local1
                    Dictionary<CompactPeptideBase, HashSet<PepsWSMods>> localCPtoPWSM = compactPeptideToProteinPeptideMatching.ToDictionary(b => b.Key, b => new HashSet<PepsWSMods>());
                    for (int i = fff.Item1; i < fff.Item2; i++)
                    {
                        foreach (var digestionParam in CollectionOfDigestionParams)
                        {
                            foreach (var peptideWithSetModifications in Proteins[i].Digest(digestionParam, FixedModifications, VariableModifications))
                            {
                                PepsWSMods pepsWithSetMods = new PepsWSMods(peptideWithSetModifications.Protein, peptideWithSetModifications.DigestionParams, peptideWithSetModifications.OneBasedStartResidueInProtein, peptideWithSetModifications.OneBasedEndResidueInProtein, peptideWithSetModifications.PeptideDescription, peptideWithSetModifications.MissedCleavages, peptideWithSetModifications.AllModsOneIsNterminus, peptideWithSetModifications.NumFixedMods);
                                if (localCPtoPWSM.TryGetValue(new CompactPeptide(peptideWithSetModifications, TerminusType), out HashSet<PepsWSMods> v))
                                {
                                    v.Add(pepsWithSetMods);
                                }
                            }
                        }
                    }
                                    ...
                                    foreach (double precursorMass in listScanPrecursorMasses)
                                    {
                                        ...
                                        foreach (DigestionParams digestionParam in CollectionOfDigestionParams)
                                        {
                                            if (index >= 0 && (pwsm.OneBasedEndResidueInProtein - (pwsm.OneBasedStartResidueInProtein + index - 2)) >= digestionParam.MinPeptideLength)
                                            {
                                                ...
                                                PepsWSMods tempPWSM = new PepsWSMods(pwsm.Protein, pwsm.DigestionParams, pwsm.OneBasedStartResidueInProtein + index - 1, pwsm.OneBasedEndResidueInProtein, pwsm.PeptideDescription, pwsm.MissedCleavages, allModsOneIsNTerminus, pwsm.NumFixedMods);
                                                double modifiedMass = finalMass[0];
                                                CompactPeptideWithModifiedMass tempCPWM = new CompactPeptideWithModifiedMass(kvp.Key, modifiedMass);
                                                tempCPWM.SwapMonoisotopicMassWithModifiedMass();
                                                if (localCPWMtoPWSM.TryGetValue(tempCPWM, out HashSet<PepsWSMods> tempPWSMHashSet))
            ...
            foreach (var psm in AllPsms)
                if (psm != null)
                {
                    foreach (KeyValuePair<CompactPeptideBase, Tuple<int, HashSet<PepsWSMods>>> kvp in psm.CompactPeptides)
                    {
                        (kvp.Key as CompactPeptideWithModifiedMass).SwapMonoisotopicMassWithModifiedMass();
                        //Change CPWM to reflect actual CP
                        if (CPWMtoPWSM.TryGetValue(kvp.Key, out HashSet<PepsWSMods> misplacedPWSMs))
                        {
                            (kvp.Key as CompactPeptideWithModifiedMass).CropTerminalMasses(TerminusType);
                            if (CPWMtoPWSM.TryGetValue(kvp.Key, out HashSet<PepsWSMods> wellPlacedPWSMs))
                            {
                                foreach (PepsWSMods PWSM in misplacedPWSMs)
        ...
        private int ComputePeptideIndexes(PepsWSMods yyy, double[] prevMass, int oneBasedIndexToLookAt, int direction, double precursorMass, MassDiffAcceptor massDiffAcceptor)
        {
