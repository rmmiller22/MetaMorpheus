...
namespace EngineLayer
{
    public class ProteinGroup
    {
        ...
        public ProteinGroup(HashSet<Protein> proteins, HashSet<PepsWSMods> peptides, HashSet<PepsWSMods> uniquePeptides)
        {
            Proteins = proteins;
        ...
        public HashSet<PepsWSMods> AllPeptides { get; set; }

        public HashSet<PepsWSMods> UniquePeptides { get; set; }
        ...
        {
            var proteinsWithUnambigSeqPsms = new Dictionary<Protein, List<PepsWSMods>>();
            var proteinsWithPsmsWithLocalizedMods = new Dictionary<Protein, List<PepsWSMods>>();

            foreach (var protein in Proteins)
            {
                proteinsWithUnambigSeqPsms.Add(protein, new List<PepsWSMods>());
                proteinsWithPsmsWithLocalizedMods.Add(protein, new List<PepsWSMods>());
            }
        ...
        public ProteinGroup ConstructSubsetProteinGroup(string fullFilePath)
        {
            ...
            var allPeptidesForThisFile = new HashSet<PepsWSMods>(allPsmsForThisFile.SelectMany(p => p.CompactPeptides.SelectMany(b => b.Value.Item2)));
            var allUniquePeptidesForThisFile = new HashSet<PepsWSMods>(this.UniquePeptides.Intersect(allPeptidesForThisFile));
