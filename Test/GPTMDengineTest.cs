...
namespace Test
{
    ...
    public static class GptmdEngineTest
    {
        ...
        public static void TestGptmdEngine()
        {
            ...
            var modPwsm = parentProtein.Digest(digestionParams, new List<ModificationWithMass>(), variableModifications).First();
            PepsWSMods modPep = new PepsWSMods(modPwsm.Protein, modPwsm.DigestionParams, modPwsm.OneBasedStartResidueInProtein, modPwsm.OneBasedEndResidueInProtein, modPwsm.PeptideDescription, modPwsm.MissedCleavages, modPwsm.AllModsOneIsNterminus, modPwsm.NumFixedMods);

            var peptidesWithSetModifications = new List<PepsWSMods> { modPep };
            ...
            {
                {peptidesWithSetModifications.First().CompactPeptide(TerminusType.None), new HashSet<PepsWSMods>{ peptidesWithSetModifications.First() } }
            };
