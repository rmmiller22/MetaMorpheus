...
namespace Test
{
    ...
    public static class LocalizationTest
    {
        ...
        public static void TestLocalization()
        {
            ...
            List<PepsWSMods> allPepsWSMods = new List<PepsWSMods>();
            foreach (var pwsm in allPeptidesWithSetModifications)
            {
                PepsWSMods pep = new PepsWSMods(pwsm.Protein, pwsm.DigestionParams, pwsm.OneBasedStartResidueInProtein, pwsm.OneBasedEndResidueInProtein, pwsm.PeptideDescription, pwsm.MissedCleavages, pwsm.AllModsOneIsNterminus, pwsm.NumFixedMods);
                allPepsWSMods.Add(pep);
            }
            Assert.AreEqual(4, allPepsWSMods.Count());
            PepsWSMods ps = allPepsWSMods.First();

            List<ProductType> lp = new List<ProductType> { ProductType.BnoB1ions, ProductType.Y };

            PepsWSMods pepWithSetModsForSpectrum = allPepsWSMods[1];
            MsDataFile myMsDataFile = new TestDataFile(new List<PepsWSMods> { pepWithSetModsForSpectrum });
            ...
            {
                {ps.CompactPeptide(TerminusType.None), new HashSet<PepsWSMods>{ ps} }
            };
